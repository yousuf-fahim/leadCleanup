                        ...
                        if st.session_state['main_processing']['df'] is not None:
                            ...
                            if not valid:
                                ...
                                # DNC PHONE NUMBER CLEANER
                                    ...
                                    processing_text.text("Initializing DNC phone number cleaning...")
                                    
                                    # Make a copy to avoid modifying the original
                                    df_copy = df.copy()
                                    
                                    # Find all potential DNC columns
                                    potential_dnc_cols = [col for col in df_copy.columns if 'DNC' in col.upper()]
                                    ...
                                    if not potential_dnc_cols:
                                        ...
                                        # Initialize session state for DNC cleaner if not exists
                                        if 'dnc_cleaner_settings' not in st.session_state:
                                            st.session_state['dnc_cleaner_settings'] = {
                                                'selected_dnc_col': 'DIRECT_DNC' if 'DIRECT_DNC' in potential_dnc_cols else potential_dnc_cols[0],
                                        ...
                                        st.session_state['dnc_cleaner_settings']['selected_dnc_col'] = selected_dnc_col
                                        
                                        # Show DNC column preview
                                        with st.expander("Preview DNC Column Data"):
                                            dnc_sample = df_copy[selected_dnc_col].fillna('MISSING').head(20)
                                            st.write("**Sample values from selected DNC column:**")
                                            for i, val in enumerate(dnc_sample):
                                                st.write(f"Row {i+1}: `{val}`")
                                        
                                        # Option to handle multiple Y/N values
                                        handle_multiple = st.checkbox(
                                            "Handle multiple Y/N values in cells",
                                            value=st.session_state['dnc_cleaner_settings']['handle_multiple'],
                                            help="When enabled, cells like 'Y,N,N' will create duplicate rows for each 'N' after removing 'Y's"
                                        )
                                        
                                        # Update session state
                                        st.session_state['dnc_cleaner_settings']['handle_multiple'] = handle_multiple
                                        
                                        # Process button
                                        process_clicked = st.button("Process DNC Cleaning", key="dnc_process_btn")
                                        
                                        # Processing logic
                                        if process_clicked or st.session_state['dnc_cleaner_settings']['processed']:
                                            if process_clicked:
                                                # Reset processed state when new processing is requested
                                                st.session_state['dnc_cleaner_settings']['processed'] = False
                                                st.session_state['dnc_cleaner_settings']['results'] = None
                                            
                                            if not st.session_state['dnc_cleaner_settings']['processed']:
                                                with st.spinner("Processing DNC phone number cleaning..."):
                                                    ...
                                                    available_phone_cols = [col for col in all_phone_cols if col in df_copy.columns]
                                                    
                                                    processing_text.text("Analyzing DNC patterns...")
                                                    
                                                    # Parse DNC values and handle multiple Y/N patterns
                                                    output_rows = []
                                                    original_rows = len(df_copy)
                                                    
                                                    for idx, row in df_copy.iterrows():
                                                        dnc_value = str(row[selected_dnc_col]).upper() if pd.notna(row[selected_dnc_col]) else 'N'
                                                        
                                                        if handle_multiple and (',' in dnc_value or ';' in dnc_value or ' ' in dnc_value):
                                                            # Handle multiple values in cell
                                                            # Split by common separators
                                                            dnc_values = []
                                                            for sep in [',', ';', ' ']:
                                                                if sep in dnc_value:
                                                                    dnc_values = [v.strip() for v in dnc_value.split(sep) if v.strip()]
                                                                    break
                                                                
                                                            if not dnc_values:
                                                                dnc_values = [dnc_value]
                                                                
                                                            # Count Y's and N's
                                                            y_count = sum(1 for v in dnc_values if v in ['Y', 'YES', 'TRUE', '1'])
                                                            n_values = [v for v in dnc_values if v in ['N', 'NO', 'FALSE', '0']]
                                                            
                                                            # If there are Y's, remove phone numbers and create rows for each N
                                                            if y_count > 0:
                                                                # Create modified row with phones removed
                                                                modified_row = row.copy()
                                                                for phone_col in available_phone_cols:
                                                                    if phone_col in modified_row:
                                                                        modified_row[phone_col] = ''
                                                                    
                                                                # Create duplicate rows for each N
                                                                if n_values:
                                                                    for n_val in n_values:
                                                                        dup_row = modified_row.copy()
                                                                        dup_row[selected_dnc_col] = n_val
                                                                        output_rows.append(dup_row)
                                                                else:
                                                                    # No N's, just add the Y row with phones removed
                                                                    modified_row[selected_dnc_col] = 'Y'
                                                                    output_rows.append(modified_row)
                                                            else:
                                                                # No Y's, keep all N rows as is
                                                                for n_val in n_values:
                                                                    dup_row = row.copy()
                                                                    dup_row[selected_dnc_col] = n_val
                                                                    output_rows.append(dup_row)
                                                                if not n_values:
                                                                    # No valid Y or N, default to N
                                                                    row_copy = row.copy()
                                                                    row_copy[selected_dnc_col] = 'N'
                                                                    output_rows.append(row_copy)
                                                        else:
                                                            # Single value processing
                                                            row_copy = row.copy()
                                                            if dnc_value in ['Y', 'YES', 'TRUE', '1']:
                                                                # Remove phone numbers for Y records
                                                                for phone_col in available_phone_cols:
                                                                    if phone_col in row_copy:
                                                                        row_copy[phone_col] = ''
                                                                row_copy[selected_dnc_col] = 'Y'
                                                            else:
                                                                # Keep phone numbers for N records
                                                                row_copy[selected_dnc_col] = 'N'
                                                            output_rows.append(row_copy)
                                                        ...
                                                        processing_text.text("Creating output dataset...")
                                                        output_df = pd.DataFrame(output_rows).reset_index(drop=True)
                                                        
                                                        progress_bar.progress(0.8)
                                                        
                                                        # Calculate statistics
                                                        final_rows = len(output_df)
                                                        dnc_y_count = sum(output_df[selected_dnc_col] == 'Y')
                                                        dnc_n_count = sum(output_df[selected_dnc_col] == 'N')
                                                        
                                                        # Count phones before and after
                                                        phones_before_cleaning = {}
                                                        phones_after_cleaning = {}
                                                        phones_removed = {}
                                                        
                                                        for col in available_phone_cols:
                                                            phones_before_cleaning[col] = sum(df_copy[col].notna() & (df_copy[col] != ''))
                                                            phones_after_cleaning[col] = sum(output_df[col].notna() & (output_df[col] != ''))
                                                            phones_removed[col] = phones_before_cleaning[col] - phones_after_cleaning[col] + (final_rows - original_rows) * sum(df_copy[col].notna() & (df_copy[col] != '')) // original_rows
                                                        
                                                        total_phones_removed = sum(phones_removed.values())
                                                        total_phones_kept = sum(phones_after_cleaning.values())
                                                        
                                                        # Store results in session state
                                                        st.session_state['dnc_cleaner_settings']['results'] = {
                                                            'output_df': output_df,
                                                            'original_rows': original_rows,
                                                            'final_rows': final_rows,
                                                            'dnc_y_count': dnc_y_count,
                                                            'dnc_n_count': dnc_n_count,
                                                            'phones_before_cleaning': phones_before_cleaning,
                                                            'phones_after_cleaning': phones_after_cleaning,
                                                            'phones_removed': phones_removed,
                                                            'total_phones_removed': total_phones_removed,
                                                            'total_phones_kept': total_phones_kept,
                                                            'available_phone_cols': available_phone_cols,
                                                            'selected_dnc_col': selected_dnc_col,
                                                            'handle_multiple': handle_multiple
                                                        }
                                                        
                                                        st.session_state['dnc_cleaner_settings']['processed'] = True
                                                        progress_bar.progress(1.0)
                                                
                                                # Display results from session state
                                                if st.session_state['dnc_cleaner_settings']['results']:
                                                    results = st.session_state['dnc_cleaner_settings']['results']
                                                    
                                                    st.success(f"✅ DNC phone number cleaning complete! Processed {results['original_rows']:,} original rows → {results['final_rows']:,} final rows")
                                                    ...
                                                    with col1:
                                                        st.metric("Original Rows", f"{results['original_rows']:,}")
                                                    with col2:
                                                        st.metric("Final Rows", f"{results['final_rows']:,}")
                                                    with col3:
                                                        st.metric("DNC 'Y' Records", f"{results['dnc_y_count']:,}")
                                                    with col4:
                                                        st.metric("DNC 'N' Records", f"{results['dnc_n_count']:,}")
                                                    
                                                    # Show row expansion details
                                                    if results['final_rows'] != results['original_rows']:
                                                        expansion_rate = ((results['final_rows'] - results['original_rows']) / results['original_rows'] * 100)
                                                        if expansion_rate > 0:
                                                            st.info(f"📈 **Row Expansion:** {results['final_rows'] - results['original_rows']:,} additional rows created ({expansion_rate:+.1f}%) due to multiple N values")
                                                        else:
                                                            st.info(f"📉 **Row Reduction:** {abs(results['final_rows'] - results['original_rows']):,} fewer rows ({expansion_rate:+.1f}%) after processing")
                                                    
                                                    # Detailed phone number statistics
                                                    with st.expander("Detailed Phone Number Statistics"):
                                                        phone_stats = []
                                                        for col in results['available_phone_cols']:
                                                            phone_stats.append({
                                                                'Column': col,
                                                                'Before Cleaning': results['phones_before_cleaning'][col],
                                                                'After Cleaning': results['phones_after_cleaning'][col],
                                                                'Net Change': results['phones_after_cleaning'][col] - results['phones_before_cleaning'][col],
                                                                'Change %': f"{((results['phones_after_cleaning'][col] - results['phones_before_cleaning'][col]) / results['phones_before_cleaning'][col] * 100) if results['phones_before_cleaning'][col] > 0 else 0:+.1f}%"
                                                            })
                                                    ...
                                                    with st.expander("Sample of Processed Data"):
                                                        sample_cols = [results['selected_dnc_col']] + results['available_phone_cols']
                                                        if 'FIRST_NAME' in results['output_df'].columns:
                                                            sample_cols = ['FIRST_NAME', 'LAST_NAME'] + sample_cols
                                                        
                                                        # Show samples of both Y and N records
                                                        dnc_y_sample = results['output_df'][results['output_df'][results['selected_dnc_col']] == 'Y'].head(5)
                                                        dnc_n_sample = results['output_df'][results['output_df'][results['selected_dnc_col']] == 'N'].head(5)
                                                        
                                                        if not dnc_y_sample.empty:
                                                            st.write("**Sample DNC 'Y' records (phone numbers removed):**")
                                                            available_sample_cols = [col for col in sample_cols if col in results['output_df'].columns]
                                                            st.dataframe(dnc_y_sample[available_sample_cols], use_container_width=True)
                                                        
                                                        if not dnc_n_sample.empty:
                                                            st.write("**Sample DNC 'N' records (phone numbers preserved):**")
                                                            available_sample_cols = [col for col in sample_cols if col in results['output_df'].columns]
                                                    ...
                                                    st.info(f"""
                                                    **Processing Summary:**
                                                    - 📋 Used DNC column: **{results['selected_dnc_col']}**
                                                    - 📞 Processed {len(results['available_phone_cols'])} phone columns: {', '.join(results['available_phone_cols'])}
                                                    - 🚫 Cleaned phone numbers from {results['dnc_y_count']:,} DNC 'Y' records
                                                    - ✅ Preserved phone numbers in {results['dnc_n_count']:,} DNC 'N' records
                                                    - {'🔄 Multiple Y/N handling: ENABLED' if results['handle_multiple'] else '📝 Single value processing: ENABLED'}
                                                    """)
                                                    ...
                                                    create_download_button(
                                                        results['output_df'],
                                                        "dnc_cleaned_advanced",
                                                        output_format.lower(),
                                                        f"Download DNC cleaned data with {len(results['output_df']):,} rows"
                                                    )
                                                    
                                                    # Reset button
                                                    if st.button("Process Different Settings", key="dnc_reset_btn"):
                                                        st.session_state['dnc_cleaner_settings']['processed'] = False
                                                        st.session_state['dnc_cleaner_settings']['results'] = None
                                                        st.rerun()
                                    ...
                                    # ADDRESS + HONWINCOME (basic version without names)
                                        ...
                                        if 'HOMEOWNER' in df.columns:
                                            honw_data.append('Ho ' + df['HOMEOWNER'].astype(str))
                                        if 'NET_WORTH' in df.columns:
                                            honw_data.append('NW ' + df['NET_WORTH'].astype(str))
                                        if 'INCOME_RANGE' in df.columns:
                                            honw_data.append('Income ' + df['INCOME_RANGE'].astype(str))
                                        
                                        df['DATA'] = ' | '.join(honw_data) if honw_data else 'No HoNWIncome data available'
                                    ...
                                    # ADDRESS + HONWINCOME & PHONE (basic version without names)
                                        ...
                                        df['DATA'] = (' | '.join(honw_data) if honw_data else 'No HoNWIncome data') + \
                                                df.apply(lambda row: ' | Phone ' + str(row['MOBILE_PHONE']) if (
                                                        row['DNC'] != 'Y' and row['MOBILE_PHONE'] != '') else '', axis=1)
